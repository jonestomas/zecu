...
function generateIdentifier(request: Request, userId?: string): string {
...
export async function checkRateLimit(
  ...
  try {
    ...
    if (insertError) {
      ...
      return { allowed: true, remaining: config.requests - currentRequests - 1, resetTime: now.getTime() + (config.window * 1000) };
    }
