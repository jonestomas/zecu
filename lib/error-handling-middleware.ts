...
export function withErrorHandling(handler: (_request: NextRequest) => Promise<NextResponse>) {
...
export function withAuthErrorHandling(handler: (_request: NextRequest) => Promise<NextResponse>) {
  return async (_request: NextRequest): Promise<NextResponse> => {
    try {
      return await handler(_request);
    } catch (error) {
      // Manejo específico para errores de autenticación
      return handleError(error, _request, undefined, 'AUTHENTICATION_ERROR');
    }
  };
}
...
export function withDatabaseErrorHandling(
  ...
  return async (_request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      return handleError(error, _request, undefined, 'DATABASE_ERROR');
    }
  };
}
...
export function handleValidationError(errors: any[], _request?: NextRequest): NextResponse {
...
export function handleRateLimitError(retryAfter: number, _request?: NextRequest): NextResponse {
  return NextResponse.json(
    {
      success: false,
