...
export async function updateUserPlan(phone: string, plan: "free" | "plus"): Promise<User> {
  const { data, error } = await supabaseAdmin
    ...
    .update({
      plan,
      plan_expires_at: plan === "plus" ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() : null,
    })
    .eq("phone", phone)
    .select()
    .single()
...
export async function createOTPCode(phone: string, code: string, expiresInMinutes = 5): Promise<OTPCode> {
...
export async function verifyOTPCode(phone: string, code: string): Promise<{ valid: boolean; otpRecord?: OTPCode }> {
  ...
  const { error: updateError } = await supabaseAdmin.from("otp_codes").update({ verified: true }).eq("id", data.id)
...
export async function invalidateAllOTPCodes(phone: string): Promise<void> {
  await supabaseAdmin.from("otp_codes").update({ verified: true }).eq("phone", phone).eq("verified", false)
}
