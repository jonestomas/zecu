...
export function handleZodError(error: any, requestId?: string): SecureError {
  const zodError = createSecureError('VALIDATION_ERROR', {
...
export function handleAuthError(error: any, requestId?: string, userId?: string): SecureError {
  const authError = createSecureError('AUTHENTICATION_ERROR', {
    errorName: error.name,
    errorMessage: error.message,
...
export function handleDatabaseError(error: any, requestId?: string, userId?: string): SecureError {
  const dbError = createSecureError('DATABASE_ERROR', {
    errorName: error.name,
    errorMessage: error.message,
    code: error.code,
...
export function handleExternalServiceError(
  ...
  const externalError = createSecureError('EXTERNAL_SERVICE_ERROR', {
    service,
    errorName: error.name,
    errorMessage: error.message,
...
export function handlePaymentError(
  ...
  const paymentError = createSecureError('PAYMENT_ERROR', {
    provider,
    errorName: error.name,
    errorMessage: error.message,
    statusCode: error.statusCode,
...
export function createSecureErrorResponse(
  ...
  const logger = request ? createLogger(request) : null;
  ...
  if (logger) {
    ...
    switch (secureError.logLevel) {
      ...
      case 'WARN':
        ...
        break;
      case 'ERROR':
        logger.error('API', 'Error occurred', new Error(secureError.technicalDetails?.errorMessage || 'Unknown error'), logData);
        break;
      case 'CRITICAL':
        logger.critical('API', 'Critical error occurred', new Error(secureError.technicalDetails?.errorMessage || 'Unknown error'), logData);
        break;
    }
  }
