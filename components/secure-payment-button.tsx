...
export function SecurePaymentButton({
  ...
  const _handlePayment = async () => {
    ...
    if (clickCount >= 5) {
      console.warn('Demasiados intentos. Recarga la página.');
      return;
    }
    ...
    try {
      ...
      const paymentUrl =
        process.env.NODE_ENV === 'development' ? data.sandboxInitPoint : data.initPoint;

      if (paymentUrl && isValidURL(paymentUrl)) {
        // Log de seguridad
        console.warn('🔒 Redirigiendo a pago seguro:', {
          plan: planId,
...
function generateBrowserFingerprint(): string {
  ...
  return window.btoa(components.join('|')).substring(0, 16);
}
