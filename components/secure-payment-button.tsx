...
export function SecurePaymentButton({ planId, planName, price, className = "", children }: SecurePaymentButtonProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const paymentUrl = process.env.NODE_ENV === "development" ? data.sandboxInitPoint : data.initPoint
      ...
      const userMessage =
        error instanceof Error && error.message.includes("preferencia")
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex items-center">
            ...
            <Shield className="ml-2 h-3 w-3 text-chart-2" />
          </div>
        </>
      )}

      {/* Indicador visual de seguridad */}
      {isSecure && <div className="absolute -top-1 -right-1 w-3 h-3 bg-chart-2 rounded-full animate-pulse" />}
    </Button>
  )
}
...
function isValidURL(url: string): boolean {
  try {
    ...
    const allowedProtocols = process.env.NODE_ENV === "development" ? ["https:", "http:"] : ["https:"]

    const allowedDomains = ["mercadopago.com", "mercadopago.com.ar", "sandbox.mercadopago.com.ar"]

    return (
      allowedProtocols.includes(parsed.protocol) && allowedDomains.some((domain) => parsed.hostname.includes(domain))
    )
  } catch {
    return false
  }
