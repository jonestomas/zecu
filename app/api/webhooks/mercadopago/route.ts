      ...
      try {
        ...
        switch (paymentInfo.status) {
          ...
          case 'cancelled':
            ...
            break;
          default:
            console.warn('Estado de pago no manejado:', paymentInfo.status);
        }
      } catch (paymentError) {
...
async function handleApprovedPayment(paymentInfo: any) {
  console.warn('✅ Pago aprobado:', paymentInfo.id);
    ...
    if (userPhone) {
      ...
      if (user) {
        ...
        if (user.id === userId) {
          ...
          console.warn(`✅ Plan Plus activado para usuario ${user.id} (${userPhone})`);
...
async function handleLegacyPaymentFlow(paymentInfo: any, planId: string) {
  console.warn('🔄 Ejecutando flujo legacy (sin autenticación previa)');
  ...
  try {
    ...
    console.warn(`Activando plan ${planId} para ${fullPhone || userEmail}`);
    ...
    if (planId === 'plus') {
      if (fullPhone) {
        ...
        if (existingUser) {
          ...
          console.warn(`✅ Plan Plus activado para: ${fullPhone}`);
        } else {
          ...
          console.warn(`✅ Nuevo usuario creado con plan Plus: ${fullPhone}`);
          ...
          console.warn(`✅ OTP enviado a ${fullPhone} para completar registro`);
        }
      } else if (userEmail) {
...
async function handleRejectedPayment(paymentInfo: any) {
  console.warn('❌ Pago rechazado:', paymentInfo.id);
...
async function handlePendingPayment(paymentInfo: any) {
  console.warn('⏳ Pago pendiente:', paymentInfo.id);
...
async function handleCancelledPayment(paymentInfo: any) {
  console.warn('🚫 Pago cancelado:', paymentInfo.id);
...
async function savePurchaseToDatabase(purchase: any) {
  ...
  console.warn('Guardando compra en la base de datos:', purchase);
}
