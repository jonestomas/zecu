...
export default function MisComprasPage() {
  ...
  const _getStatusIcon = (status: Purchase['status']) => {
    switch (status) {
      case 'approved':
  ...
  const _getStatusBadge = (status: Purchase['status']) => {
  ...
  const _getPlanDisplayName = (planId: string) => {
                  ...
                  {purchase.status === 'rejected' && (
                    <div className='mt-4 p-3 bg-red-50 border border-red-200 rounded-lg'>
                      <p className='text-sm text-red-800'>
                        ✗ El pago fue rechazado. Por favor, intenta nuevamente con otro método de
                        pago.
                      </p>
                    </div>
                  )}
