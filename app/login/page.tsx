...
export default function LoginPage() {
  ...
  const _otpInputs = useRef<(HTMLInputElement | null)[]>([]);
  ...
  const _handlePhoneSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    ...
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        throw new Error(
          'Error del servidor. Por favor, verifica que Supabase esté configurado correctamente.'
        );
      }
      ...
      console.warn(
        error instanceof Error ? error.message : 'Error al enviar código de verificación'
      );
      setIsLoading(false);
    }
  };
  ...
  const _handleOtpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    ...
    try {
      ...
      console.warn('✅ OTP verificado:', data);
      ...
      console.warn(error instanceof Error ? error.message : 'Código inválido o expirado');
      setIsLoading(false);
    }
  };

  const _handleNameSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    ...
    try {
      ...
      console.warn('✅ Perfil actualizado:', data);
      ...
      console.warn(error instanceof Error ? error.message : 'Error al guardar tu información');
      setIsLoading(false);
    }
  };

  const _handleOtpKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace' && !otp[index] && index > 0) {
      otpInputs.current[index - 1]?.focus();
    }
  };

  const _handlePlanSelection = async (planId: 'free' | 'plus') => {
    setIsLoading(true);
    ...
    try {
      if (planId === 'free') {
        ...
        console.warn('✅ Plan Free activado:', data);
      ...
      console.warn(error instanceof Error ? error.message : 'Error al procesar tu selección');
      setIsLoading(false);
    }
  };
