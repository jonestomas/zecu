...
export default function VerifyPage() {
  ...
  const _inputRefs = useRef<(HTMLInputElement | null)[]>([]);
  ...
  const _handleResend = async () => {
    ...
    try {
      ...
      console.warn(error instanceof Error ? error.message : 'Error al reenviar código');
      setCanResend(true);
    }
  };
  ...
  const _handleCodeChange = (index: number, value: string) => {
    if (value.length > 1) {
      value = value[0];
    }
  ...
  const _handleKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  const _handleNameSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn(error instanceof Error ? error.message : 'Error al guardar tu nombre');
      setIsLoading(false);
    }
  };

  const _isCodeComplete = code.every(digit => digit !== '');
